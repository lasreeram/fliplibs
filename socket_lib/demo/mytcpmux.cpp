...
static void parsetab( servtab_t* stp ){
	...
	if ( fp == NULL )
		sockets_lib::throw_error( "unable to open config file %s", CONFIG );
	...
	char line[MAXLINE] = {0};
	char* cp;
	sockets_lib::log( "start parsing config file" );
	while( fgets( line, sizeof(line), fp ) != NULL ){
		lineno++;
		if ( line[ strlen(line) - 1 ] != '\n' ){
			sockets_lib::throw_error( "line %d is too long", lineno );
		}
		line[ strlen(line) ] = '\0';

		sockets_lib::log( "checking if exceeded table entries" );
		if ( stp >= service_table + NSERVTAB ) 
			sockets_lib::throw_error( "too many entries in config" );
		
		sockets_lib::log( "checking for comments" );
		cp = strchr( line, '#' );
		if ( cp != NULL )
			*cp = '\0';

		sockets_lib::log( "parsing line %s into tokens", line );
		cp = strtok_r( line, " \t\n", &saveptr);
		if ( cp == NULL )
			continue;

		sockets_lib::log( "looking for + character" );
		if ( *cp == '+' ){
			stp->flag = true;
			cp++;
			if ( *cp == '\0' ||  strchr( " \t\n", *cp ) != NULL )
				sockets_lib::throw_error( "white space after + at %d", lineno );
		}

		stp->service = strdup( cp );
		if ( stp->service == NULL )
			sockets_lib::throw_error( "out of memory" );

		cp = strtok_r( NULL, " \t\n", &saveptr );

		if ( cp == NULL )
			sockets_lib::throw_error( "line missing path name, service = %s, line = %d", 
							stp->service, lineno );
		...
		if ( i >= MAXARGS && strtok_r (NULL, " \t\n", &saveptr) != NULL )
			sockets_lib::throw_error( "too many arguments at line %d", lineno );

		stp->args[i] = NULL;
		stp++;
	}
	sockets_lib::log( "finished parsing config file" );
...
void start_server( TCPSocket* socket ){
	...
	sockets_lib::log( "waiting to read a line from accept socket" );
	...
	sockets_lib::log( "matching the service from table" );
	for ( stp = service_table; stp->service; stp++ ){
		sockets_lib::log( "matching the service %s, line = %s", stp->service, line );
		if ( strcmp( line, stp->service ) == 0 ){
			sockets_lib::log( "matching %s, %s", line, stp->service );
			break;
		}else{
			sockets_lib::log( "not matching %s, %s", line, stp->service );
		}
	}

	sockets_lib::log( "checking service data" );
	if ( !stp->service ){
		sockets_lib::log( "cannot find service" );
	...
	sockets_lib::log( "executing new process path=%s, args=%s", stp->path, stp->args[0] );
	...
	if ( rc < 0 )
		sockets_lib::throw_error( "error executing process at %s, with args %s, %d, %s", 
...
void reaper( int sig ){
	...
	sockets_lib::log( "caught signal sig = %d", sig );
	int waitstatus;
...
int main( int argc, char** argv ){
	...
	if ( argc == 3 ){
		...
		sockets_lib::log( "usage: %s [ interface [ port ] ]\n", argv[1] );
	...
	try{
		...
		for( ;; ){
			sockets_lib::log( "listening for connection" );
			TCPSocket* acceptsocket = listensocket->accept();			
			sockets_lib::log( "created an accept socket" );
